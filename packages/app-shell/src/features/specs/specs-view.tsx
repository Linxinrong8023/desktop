...
export const SpecsContent = forwardRef<SpecsContentHandle, SpecsContentProps>(
  function SpecsContent({ projectId, taskId, onRefreshingChange }, ref) {
    ...
    const targetKey =
      taskId === undefined ? `project:${projectId}` : `task:${taskId}`;
    ...
    const documents = useMemo(() => {
      ...
      return all.filter((document) =>
        document.relativePath.toLowerCase().includes(debouncedFilter),
      );
    ...
    const documentQuery = useQuery({
      ...
      queryFn: ({ signal }) =>
        client.spec.read(
          { target, relativePath: activeSelectedPath! },
          { signal },
        ),
    ...
    const catalogPaths = useMemo(
      () =>
        new Set(
          (catalogQuery.data?.documents ?? []).map(
    ...
    const markdownComponents: Components = {
      a: ({ href, children, ...props }) => {
        const destination =
          href === undefined || activeSelectedPath === null
            ? null
            ...
            {activeSelectedPath !== null && (
              <div className="flex h-10 shrink-0 items-center gap-2 border-b border-border px-3">
                ...
                <span className="text-[10px] text-muted-foreground">
                  {activeSelectedPath.toLowerCase().endsWith(".mdx")
                    ? "MDX"
                    : "MD"}
                </span>
                {selectedDocument && (
                  <span className="text-[11px] text-muted-foreground">
                    {selectedDocument.byteSize.toLocaleString()} B
                  </span>
                )}
                <Button
                  size="icon-sm"
                  variant={mode === "preview" ? "secondary" : "ghost"}
                  aria-label={t("specs.preview")}
          ...
          <div className="flex h-full min-h-0 flex-col border-l border-border">
            ...
            <div className="min-h-0 flex-1">
              {catalogQuery.isLoading ? (
                <Status text={t("specs.loading")} />
              ) : catalogQuery.error !== null ? (
                <Status
                  text={localizeContractError(catalogQuery.error, t)}
                  destructive
                />
              ) : documents.length === 0 ? (
            ...
            {catalogQuery.data?.truncated && (
              <p className="border-t border-border px-3 py-2 text-[11px] text-amber-700">
                {t("specs.truncated")}
              </p>
            )}
          </div>
...
function SpecDocumentBody({
  ...
  if (error !== null)
    return <Status text={localizeContractError(error, t)} destructive />;
...
function Status({
  ...
  return (
    <div
      data-selectable
      className={`flex h-full items-center justify-center p-6 text-sm ${destructive ? "text-destructive" : "text-muted-foreground"}`}
    >
      {text}
    </div>
  );
