...
export function DiffView({ path, oldText, newText }: DiffViewProps) {
  ...
  const lines = useMemo(
    () => buildDisplayLines(oldText ?? "", newText),
    [oldText, newText],
  );
    ...
    <div className="overflow-hidden rounded-md border border-border bg-background">
      ...
      <div className="overflow-x-auto font-mono text-[11px] leading-5">
        {visibleLines.map((line, index) => (
          ...
          >
            <span className="w-10 shrink-0 select-none border-r border-border/50 px-2 text-right text-muted-foreground/70">
              {line.oldLine ?? ""}
            </span>
            <span className="w-10 shrink-0 select-none border-r border-border/50 px-2 text-right text-muted-foreground/70">
              {line.newLine ?? ""}
            ...
            <span data-selectable className="pr-4 whitespace-pre">
              {line.text}
            </span>
          </div>
        ))}
...
function selectChangedHunks(lines: DisplayLine[]): DisplayLine[] {
  ...
  if (visibleIndexes.size === 0 || visibleIndexes.size === lines.length)
    return lines;
