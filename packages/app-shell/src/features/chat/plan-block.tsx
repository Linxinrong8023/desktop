...
export function PlanBlock({ plan }: PlanBlockProps) {
  ...
  const allCompleted =
    plan.entries.length > 0 && completed === plan.entries.length;
  ...
  return (
    <Collapsible
      ...
      onOpenChange={(nextOpen) =>
        setDisclosure({ allCompleted, open: nextOpen })
      }
    ...
    >
      <CollapsibleTrigger className="flex min-h-11 w-full items-center gap-2 px-3 py-2 text-left text-xs outline-none transition-colors hover:bg-muted/30 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">
        ...
        <span className="text-muted-foreground">
          {t("chat.planProgress", { completed, total: plan.entries.length })}
        </span>
        <IconChevronDown
...
function PlanStatusIcon({ status }: { status: acp.PlanEntryStatus }) {
  switch (status) {
    case "pending":
      return (
        <IconCircle className="mt-1 size-3 shrink-0 text-muted-foreground" />
      );
    case "in_progress":
      return (
        <IconLoader2 className="mt-1 size-3 shrink-0 animate-spin text-sky-600 motion-reduce:animate-none" />
      );
    case "completed":
