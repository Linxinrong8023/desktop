...
const previewMarkdownComponents: Components = {
  a: ({ children }) => (
    <span className="underline decoration-foreground/30 underline-offset-2">
      {children}
    </span>
  ),
  blockquote: ({ children }) => (
    <div className="my-1 border-l border-foreground/20 pl-2 text-foreground/80">
      {children}
    </div>
  ),
  code: ({ children, className }) =>
    className === undefined ? (
      ...
      <code className="whitespace-pre-wrap break-words font-mono text-foreground/85">
        {children}
      </code>
    ),
  ...
  ol: ({ children }) => (
    <ol className="my-0.5 list-decimal pl-4">{children}</ol>
  ),
  p: ({ children }) => (
  ...
  table: ({ children }) => (
    <table className="my-1 w-full table-fixed text-[11px]">{children}</table>
  ),
  td: ({ children }) => (
    <td className="truncate border-t border-foreground/10 pr-1">{children}</td>
  ),
  th: ({ children }) => (
    <th className="truncate pr-1 text-left font-medium">{children}</th>
  ),
...
export function ConversationPreviewMarkdown({
  content,
}: ConversationPreviewMarkdownProps) {
