...
function WebPathPickerDialog({
  ...
  const [directory, setDirectory] = useState<ListDirectoryResponse | null>(
    null,
  );
  ...
  const selectedEntry =
    selectedIndex === -1 ? undefined : entries[selectedIndex];
  const confirmableEntry =
    selectedEntry !== undefined && selectedEntry.kind === options.kind
      ? selectedEntry
      : undefined;
  const selectionPath =
    confirmableEntry?.path ??
  ...
  const title =
    options.kind === "directory"
      ? messages.chooseDirectoryTitle
      : messages.chooseFileTitle;
  ...
  const activeDescendant =
    selectedIndex === -1 ? undefined : `platform-path-entry-${selectedIndex}`;

  return (
    <Dialog
      open
      onOpenChange={(open) =>
        !open && adapter.completeSelection(requestId, null)
      }
    >
      <DialogContent
      ...
      >
        <DialogHeader
          ...
          style={{
            padding: `${PATH_PICKER_GUTTER} ${PATH_PICKER_GUTTER} 0.75rem`,
          }}
        >
        ...
        >
          <Button
            ...
            onClick={() =>
              directory?.parentPath != null && navigateTo(directory.parentPath)
            }
        ...
        >
          {!loading && readError && (
            <div
              role="alert"
              className="grid h-full place-items-center gap-3 p-6 text-center"
            >
              <p data-selectable className="text-sm text-destructive">
                {messages.readError}
              </p>
              <Button
                type="button"
                variant="outline"
                onClick={() =>
                  void loadDirectory(pathDraft.trim() || undefined)
                }
              >
                <IconRefresh />
                {messages.retry}
              ...
              {positionedItems.map((virtualItem) => {
                ...
                return (
                  <div
                    ...
                    onDoubleClick={() =>
                      entry.kind !== "unavailable" && activateEntry(entry)
                    }
                  >
          ...
          {loading && (
            <div
              className="absolute inset-0 grid place-items-center bg-muted/55"
              aria-live="polite"
            >
        ...
        <DialogFooter
          ...
          style={{
            paddingInline: PATH_PICKER_GUTTER,
            paddingBottom: PATH_PICKER_GUTTER,
          }}
        >
          <Button
            type="button"
            variant="outline"
            onClick={() => adapter.completeSelection(requestId, null)}
          >
            {messages.cancel}
          </Button>
          <Button
            ...
            onClick={() =>
              ...
              adapter.completeSelection(requestId, selectionPath)
            }
          >
            {messages.choose}
          </Button>
