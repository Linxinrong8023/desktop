    ...
    async send<TResponse>(
      ...
      try {
        response = await fetchImplementation(
          resolveUrl(options.baseUrl ?? "", request.path),
          {
...
async function* readNdjsonStream<TEvent>(
  ...
  try {
    ...
    const response = await fetchImplementation(
      resolveUrl(baseUrl, request.path),
      {
        method: request.method,
    ...
    try {
      while (!ended) {
        ...
        if (
          new TextEncoder().encode(buffered).byteLength > MAX_FRAME_BYTES &&
          !buffered.includes("\n")
        ) {
          throw streamError(
            "stream_frame_too_large",
            "contract stream frame exceeds 8 MiB",
          );
        }
      ...
      if (!ended)
        throw streamError(
          "stream_interrupted",
          "contract stream closed before its end frame",
        );
    } finally {
...
function decodeStreamFrame<TEvent>(line: string): ContractStreamFrame<TEvent> {
  ...
  try {
    ...
    throw streamError(
      ...
      "contract stream emitted invalid JSON",
    );
  }
  if (!isRecord(value)) {
    throw streamError(
      "malformed_stream_frame",
      "contract stream emitted an invalid frame",
    );
  }
  if (value.type === "data" && "data" in value) {
    return value as ContractStreamFrame<TEvent>;
  }
  if (value.type === "error" && isRecord(value.error) && "error" in value) {
    return value as ContractStreamFrame<TEvent>;
  }
  if (value.type === "end") {
    return value as ContractStreamFrame<TEvent>;
