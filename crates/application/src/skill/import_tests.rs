...
fn rejects_upload_without_manifest_and_discards_staging() {
    ...
    assert_eq!(error, ApplicationError::SkillManifestMissing);
    assert_eq!(store.staging_ids(), Vec::<String>::new());
...
fn rejects_unsafe_manifest_name() {
    ...
    assert_eq!(error, ApplicationError::SkillManifestNameInvalid);
}
...
fn rejects_empty_unsafe_and_duplicate_uploads_before_staging() {
    ...
    assert_eq!(empty, ApplicationError::SkillUploadEmpty);
    assert_eq!(unsafe_path, ApplicationError::SkillUploadPathInvalid);
    assert_eq!(duplicate, ApplicationError::SkillUploadPathDuplicate);
...
fn rolls_back_row_and_discards_staging_when_promote_fails() {
    ...
    assert_eq!(
        ...
        ApplicationError::SkillPackageStorage {
            source: SkillPackageStoreError::new(std::io::Error::other("promote failed")),
        }
    );
...
fn removes_promoted_directory_when_commit_fails() {
    ...
    assert_eq!(
        ...
        ApplicationError::SkillRepository {
            source: RepositoryError::new(std::io::Error::other("commit failed")),
        }
    );
    assert_eq!(unit_of_work.committed_rows(), Vec::<Skill>::new());
...
impl SkillPackageStore for Rc<FakeSkillPackageStore> {
    ...
    fn promote(&self, skill_id: &SkillId, name: &str) -> Result<(), SkillPackageStoreError> {
        ...
        if state.fail_promote {
            return Err(SkillPackageStoreError::new(std::io::Error::other(
                "promote failed",
            )));
        }
        let files = state
...
impl SkillImportUnitOfWork for FakeUnitOfWork {
        ...
        OnInserted: FnOnce() -> Result<(), SkillPackageStoreError>,
    {
        ...
        match on_inserted() {
            ...
            Err(source) => Err(SkillImportCommitError::Promote { source }),
        }
    }
}
