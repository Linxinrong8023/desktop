...
use super::persistence::{map_operation, update_operation};
...
pub(super) fn load_unfinished_operations(
    ...
    repository.pool.with_connection(|connection| {
        let mut statement = connection.prepare(
            "SELECT id, attempt_id, resource_id, generation, sequence, mutation,
                    ...
                    prepared_at, applied_at, finalized_at, updated_at
...
pub(super) fn quarantine_unfinished_operations(
    repository: &SqliteEffectRepository,
    ...
    repository.pool.with_connection_mut(|connection| {
        ...
        let operations = {
            let mut statement = transaction.prepare(
                "SELECT operation.id, operation.attempt_id, operation.resource_id,
                        ...
                        operation.finalized_at, operation.updated_at, attempt.target_id
        ...
        for (mut operation, target) in operations.iter().cloned() {
            ...
            transaction.execute(
                "UPDATE effect_reconcile_attempts
                 SET phase = 'recovery_required', updated_at = ?2
                 WHERE id = ?1 AND phase <> 'finalized'",
                params![operation.attempt().as_str(), detected_at.millis()],
            )?;
            transaction.execute(
                "UPDATE effect_operation_artifacts SET state = 'retained', updated_at = ?2
                 WHERE operation_id = ?1 AND state = 'reserved'",
                params![operation.identity().as_str(), detected_at.millis()],
            )?;
            target_recoveries
                .entry(target)
        ...
        for (target, (operation, generation)) in &target_recoveries {
            ...
            status
                .require_recovery(operation.clone(), detected_at)
                .map_err(|error| DatabaseError::CorruptEffectState(error.to_string()))?;
            ...
            let updated = transaction.execute(
                "UPDATE effect_reconcile_requests
                 SET state = 'blocked', claim_token = NULL, claim_worker = NULL,
                     ...
                     resume_trigger_json = '{\"kind\":\"manual_recovery\"}', updated_at = ?3
                 WHERE target_id = ?1",
                params![
                    target.as_str(),
...
pub(super) fn complete_artifact_cleanup(
    repository: &SqliteEffectRepository,
...
pub(super) fn mark_artifact_cleanup_failed(
    repository: &SqliteEffectRepository,
    ...
    repository.pool.with_connection(|connection| {
        let updated = connection.execute(
            "UPDATE effect_operation_artifacts
             SET state = 'cleanup_failed', updated_at = ?3
