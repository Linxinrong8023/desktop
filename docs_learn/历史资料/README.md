...
- [学习使命](MISSION.md)
- [可信资料索引](RESOURCES.md)
- 插件系统速查图（旧 HTML 页面已删除）
- [Agent 插件学习问答：40 题复习与当前实现校正](lessons/agent-plugin-qa.md)
- 专题第 1 课：插件到底是什么（旧 HTML 页面已删除）
- 专题第 2 课：插件文件、插件进程与插件数据（旧 HTML 页面已删除）
- 专题第 3 课：Manifest 是插件的身份证（旧 HTML 页面已删除）
- 专题第 4 课：Manifest 与 Plugin Manager 的两阶段验证（旧 HTML 页面已删除）
- 专题第 5 课：InstalledPlugin 是验证后的可信宿主视图（旧 HTML 页面已删除）
- 专题第 6 课：PluginContribution 与类型化能力（旧 HTML 页面已删除）
- 专题第 7 课：Contribution 不等于 Runtime（旧 HTML 页面已删除）
- 专题第 8 课：Agent 插件的双进程模型（旧 HTML 页面已删除）
- 专题第 9 课：Plugin JSON-RPC 与 ACP 的协议分层（旧 HTML 页面已删除）
- 专题第 10 课：一条 Prompt 的端到端协议路径（旧 HTML 页面已删除）
- 专题第 11 课：ora/register 与运行时能力合同（旧 HTML 页面已删除）
- 专题第 12 课：Plugin ID 与 Process Generation（旧 HTML 页面已删除）
- 专题第 13 课：Plugin Lifecycle 是唯一进程所有者（旧 HTML 页面已删除）
- 专题第 14 课：插件 Running 不等于 Agent Ready（旧 HTML 页面已删除）
- Agent 插件主线阶段总结（旧 HTML 页面已删除）
- 专题第 15 课：connection 与 ensure_running（旧 HTML 页面已删除）
- 专题第 16 课：为什么 Agent 需要 Runtime 与 Supervisor（旧 HTML 页面已删除）
- 专题第 17 课：一个 Agent 如何准确服务多个 Session（旧 HTML 页面已删除）
- 专题第 18 课：失败不是一种失败（旧 HTML 页面已删除）
- 专题第 19 课：Agent 插件到底安全吗（旧 HTML 页面已删除）
- 专题第 20 课：启动与退出必须对称（旧 HTML 页面已删除）
- 专题第 21 课：Agent Ready 前的三次确认（旧 HTML 页面已删除）
- 专题第 22 课：PluginId 与 AgentRef（旧 HTML 页面已删除）
- 专题第 23 课：三个 Generation 不是同一个计数器（旧 HTML 页面已删除）
- 专题第 24 课：模型发现为什么有两条路径（旧 HTML 页面已删除）
- 专题第 25 课：Ora 运行中安装与卸载如何生效（旧 HTML 页面已删除）
- 专题第 26 课：defineAgent 与插件作者的分工（旧 HTML 页面已删除）
- Agent 插件学习覆盖审计：已掌握、待验收与遗漏（旧 HTML 页面已删除）
