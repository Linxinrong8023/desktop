...
## 〇、用户视角（Spec 管理是什么功能，白话版）

- **本质**：一个“专门看规格 md 的只读浏览器”——把散落在约定目录里的 md 文档自动发现、按源分组、目录树展示、只读预览；
- **用户能做什么**：选中项目/任务 → Files 面板 → Specs 子视图 → 看目录树 → 点文档预览（Markdown 渲染 / 源码视图）→ Manage sources 配置哪些目录算源；
- **不能做什么**：不能创建/编辑/删除文档（文档属于用户自己的文件系统工具）；
- **有用场景**：对照开发（边写代码边看需求）、审查产出（规格 vs 实现）、评审变更提案；
- **为什么只读**：规格文档有自己的生命周期（OpenSpec：提案→审批→合并），编辑权属于那套流程，Ora 不绕过；
- **诚实标注**：不用 OpenSpec/Superpowers 工作流的用户基本用不到——它是为特定用户准备的“阅读模式”，不是通用功能；
- **与 Explorer 的区别**：Explorer 也能看 md，但 Specs 专为规格组织（自动发现约定目录 + 按源分组 + 安全渲染）。
